...
public class PersistenceTests {
  ...
  public void persistenceTest() throws TestException {
    ...
    Assertions.assertEquals(
        200, serverFacade.getStatusCode(), "Server response code was not 200 OK");
    Assertions.assertEquals(
        1, listResult.games.length, "Missing game(s) in database after restart");

    TestModels.TestListResult.TestListEntry game1 = listResult.games[0];
    Assertions.assertEquals(game1.gameID, createResult.gameID);
    Assertions.assertEquals(
        createRequest.gameName, game1.gameName, "Game name changed after restart");
    Assertions.assertEquals(
  ...
  private int getDatabaseRows() {
    ...
    try {
      ...
      Assertions.fail(
          "Unable to load database in order to verify persistence. Are you using"
              + " dataAccess.DatabaseManager to set your credentials?");
    }
