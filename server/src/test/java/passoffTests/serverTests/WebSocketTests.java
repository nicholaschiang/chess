...
public class WebSocketTests {
  ...
  public void joinPlayerGood() {
    ...
    Map<String, List<TestModels.TestMessage>> messages =
        joinPlayer(
            white.user, white.authToken, gameID, ChessGame.TeamColor.WHITE, Set.of(), Set.of());
  ...
  public void joinPlayerSteal() {
    ...
    Map<String, List<TestModels.TestMessage>> messages =
        joinPlayer(
            white.user, white.authToken, gameID, ChessGame.TeamColor.BLACK, Set.of(), Set.of());
  ...
  public void joinPlayerEmpty() {
    ...
    TestModels.TestCreateResult createResult =
        serverFacade.createGame(createRequest, white.authToken);
  ...
  public void joinPlayerBadGameID() {
    Map<String, List<TestModels.TestMessage>> messages =
        joinPlayer(
            white.user, white.authToken, gameID + 1, ChessGame.TeamColor.WHITE, Set.of(), Set.of());
  ...
  public void joinObserverGood() {
    ...
    messages =
        joinObserver(
            observer.user, observer.authToken, gameID, Set.of(white.user, black.user), Set.of());
  ...
  public void validMove() {
    ...
    Map<String, List<TestModels.TestMessage>> messages =
        makeMove(
            white.user, white.authToken, gameID, move, Set.of(black.user, observer.user), Set.of());
  ...
  public void invalidMoveBadMove() {
    ...
    Map<String, List<TestModels.TestMessage>> messages =
        makeMove(
            white.user, white.authToken, gameID, move, Set.of(), Set.of(black.user, observer.user));

    assertErrorMessage(messages.get(white.user));
    Assertions.assertTrue(
        messages.get(black.user).isEmpty(),
        "Player got a message after the other player sent an invalid command");
    Assertions.assertTrue(
        messages.get(observer.user).isEmpty(),
  ...
  public void invalidMoveWrongTurn() {
    ...
    Map<String, List<TestModels.TestMessage>> messages =
        makeMove(
            black.user, black.authToken, gameID, move, Set.of(), Set.of(white.user, observer.user));

    assertErrorMessage(messages.get(black.user));
    Assertions.assertTrue(
        messages.get(white.user).isEmpty(),
        "Player got a message after the other player sent an invalid command");
    Assertions.assertTrue(
        messages.get(observer.user).isEmpty(),
        "Observer got a message after player sent an invalid command");
  ...
  public void invalidMoveGameOver() {
    ...
    makeMove(
        black.user, black.authToken, gameID, move, Set.of(white.user, observer.user), Set.of());
  ...
  public void validResign() {
    ...
    Map<String, List<TestModels.TestMessage>> messages =
        resign(white.user, white.authToken, gameID, Set.of(black.user, observer.user), Set.of());
  ...
  public void invalidResignObserver() {
    ...
    Map<String, List<TestModels.TestMessage>> messages =
        resign(observer.user, observer.authToken, gameID, Set.of(), Set.of(white.user, black.user));
  ...
  public void invalidResignGameOver() {
    ...
    Map<String, List<TestModels.TestMessage>> messages =
        resign(white.user, white.authToken, gameID, Set.of(), Set.of(black.user, observer.user));
  ...
  public void leaveGame() {
    ...
    Map<String, List<TestModels.TestMessage>> messages =
        leave(white.user, white.authToken, gameID, Set.of(black.user, observer.user), Set.of());
  ...
  public void multipleLobbies() {
    ...
    messages =
        joinObserver(
            ...
            observer2.authToken,
            otherGameID,
            Set.of(white2.user, black2.user),
            Set.of(white.user, black.user, observer.user));
    ...
    assertMoveMadePair(messages.get(observer.user));
    Assertions.assertTrue(
        messages.get(white2.user).isEmpty(),
        "Player got a message after player from another lobby sent a command");
    Assertions.assertTrue(
        messages.get(black2.user).isEmpty(),
        "Player got a message after player from another lobby sent a command");
    Assertions.assertTrue(
        messages.get(observer2.user).isEmpty(),
        "Observer got a message after player from another lobby sent a command");
    ...
    messages =
        resign(
            ...
            white2.authToken,
            otherGameID,
            Set.of(black2.user, observer2.user),
            Set.of(white.user, black.user, observer.user));
    assertNotificationMessage(messages.get(white2.user));
    assertNotificationMessage(messages.get(black2.user));
  ...
  private Map<String, List<TestModels.TestMessage>> joinPlayer(
  ...
  private Map<String, List<TestModels.TestMessage>> joinObserver(
      String sendingUsername,
      String sendingAuth,
      int gameID,
      Set<String> recipients,
  ...
  private Map<String, List<TestModels.TestMessage>> makeMove(
      String sendingUsername,
      String sendingAuth,
      int gameID,
      ChessMove move,
  ...
  private Map<String, List<TestModels.TestMessage>> resign(
      String sendingUsername,
      String sendingAuth,
      int gameID,
      Set<String> recipients,
  ...
  private Map<String, List<TestModels.TestMessage>> leave(
      String sendingUsername,
      String sendingAuth,
      int gameID,
      Set<String> recipients,
  ...
  private void assertLoadGameMessage(List<TestModels.TestMessage> messages) {
    ...
    Assertions.assertEquals(
        TestModels.TestServerMessageType.LOAD_GAME,
        messages.get(0).serverMessageType,
  ...
  private void assertNotificationMessage(List<TestModels.TestMessage> messages) {
    Assertions.assertEquals(1, messages.size(), "Expected 1 message, got " + messages.size());
    Assertions.assertEquals(
        TestModels.TestServerMessageType.NOTIFICATION,
        messages.get(0).serverMessageType,
        "Message was not a NOTIFICATION message");
    Assertions.assertNotNull(
        messages.get(0).message, "Bobs NOTIFICATION message did not contain a message");
  }

  private void assertErrorMessage(List<TestModels.TestMessage> messages) {
    Assertions.assertEquals(1, messages.size(), "Expected 1 message, got " + messages.size());
    Assertions.assertEquals(
        TestModels.TestServerMessageType.ERROR,
        messages.get(0).serverMessageType,
        "Message was not an ERROR message");
    Assertions.assertNull(messages.get(0).game, "ERROR message contained a game");
    Assertions.assertNotNull(
        messages.get(0).errorMessage, "ERROR message did not contain an error message");
  }
  ...
  private void assertMoveMadePair(List<TestModels.TestMessage> messages) {
    ...
    boolean isLoadGameFirst =
        messages.get(0).serverMessageType == TestModels.TestServerMessageType.LOAD_GAME;
    Assertions.assertEquals(
        TestModels.TestServerMessageType.LOAD_GAME,
        messages.get(isLoadGameFirst ? 0 : 1).serverMessageType,
        "Didn't get load game message");
    Assertions.assertNotNull(
        messages.get(isLoadGameFirst ? 0 : 1).game, "LOAD_GAME message didn't contain a game");
    Assertions.assertEquals(
        TestModels.TestServerMessageType.NOTIFICATION,
        messages.get(isLoadGameFirst ? 1 : 0).serverMessageType,
        "Didn't get notification message");
    Assertions.assertNotNull(
        messages.get(isLoadGameFirst ? 1 : 0).message,
